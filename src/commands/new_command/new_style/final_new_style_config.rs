...
use crate::{template::{Template, template_variables::TemplateVariables}, user_config::{UserConfig, UserNewStyleConfig}, CreateableFileType, helpers::file_helper};
...
impl FinalNewStyleConfig {
    pub fn new(style_args: &ArgMatches) -> Result<Self, String> {
        ...
        let filestem = path_arg.file_stem().ok_or(format!("Must specify the stylesheet's name"))?;
        ...
        let style_final_path =
            Self::get_style_final_path(path_arg.to_owned(), style_extension, filestem, destination_folder)?;
